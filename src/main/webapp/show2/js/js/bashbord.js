...
function getNowClueCnt(param){
	$.ajax({
		...
		success:function(data){
			...
			var logoncnt=data.result.logons;
			var clueFroms=data.result.clueFroms;
			var topicDesks=data.result.topicDesks;
			var topicRefcnt=data.result.topicRefcounts;
			clueCnt(assetByTimes)
			clueSite(clueSites);
			clueFrom(clueFroms);
			getLogon(logoncnt);
			topicDesck(topicDesks);
			topicRefcount(topicRefcnt);
			titlePlatform();
			titleOrgcount();
